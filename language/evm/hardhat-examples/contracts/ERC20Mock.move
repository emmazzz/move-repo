...
module Evm::ERC20Mock {
    ...
    public fun burn(account: address, amount: U256) acquires State {
        burn_(account, amount)
    }

    #[callable(sig=b"transferInternal(address,address,uint256)")]
    public fun transferInternal(from: address, to: address, value: U256) acquires State {
        transfer_(from, to, value)
    }

    #[callable(sig=b"approveInternal(address,address,uint256)")]
    public fun approveInternal(owner: address, spender: address, value: U256) acquires State {
        approve_(owner, spender, value)
    }
    // ----------------------------------------
    ...
    public fun create(name: vector<u8>, symbol: vector<u8>, initial_account: address, initial_balance: U256) acquires State {
        ...
        mint_(initial_account, initial_balance);
    }
